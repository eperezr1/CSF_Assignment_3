...
int main(int argc, char **argv) {
  ...
  if (alloc == "no-write-allocate" && through_back == "write-back") {
    std::cerr << "Invalid combination of no-write-allocate and write-back "
                 "specified\n";
    return 1;
  }
  ...
  // read trace file line by line (file given as input to program)  --- make
  // this a cache method instead?
  std::string cur_line;
